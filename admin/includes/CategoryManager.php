...
class CategoryManager {
    ...
    public function getCategories($status = 'active') {
        $categories = $this->db->readCSV('categories.csv');
        if ($status) {
    ...
    public function getCategory($id) {
        return $this->db->findById('categories.csv', $id);
    }
    ...
    public function saveCategory($data, $isUpdate = false) {
        ...
        if ($isUpdate) {
            return $this->db->updateById('categories.csv', $data['id'], $csvData);
        } else {
            // Check if ID already exists
            if ($this->db->findById('categories.csv', $data['id'])) {
                return false; // ID already exists
            }
            
            $headers = ['id', 'name', 'icon', 'description', 'status', 'created_date', 'updated_date'];
            return $this->db->appendCSV('categories.csv', $csvData, $headers);
        }
    }
    
    public function deleteCategory($id) {
        return $this->db->deleteById('categories.csv', $id);
    }
}
?>
